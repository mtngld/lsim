...
class LsimModel(object):
    ...
    def build_vgg(self, branch):
        ...
        with tf.variable_scope('decoder'):
            ...
            self.disp4[branch] = self.get_disp(iconv4)
            udisp4  = self.upsample_nn(self.disp4[branch], 2)

            upconv3 = upconv(iconv4,  64, 3, 2) #H/4
            concat3 = tf.concat([upconv3, skip2, udisp4], 3)
            iconv3  = conv(concat3,   64, 3, 1)
            self.disp3[branch] = self.get_disp(iconv3)
            udisp3  = self.upsample_nn(self.disp3[branch], 2)

            upconv2 = upconv(iconv3,  32, 3, 2) #H/2
            concat2 = tf.concat([upconv2, skip1, udisp3], 3)
            iconv2  = conv(concat2,   32, 3, 1)
            self.disp2[branch] = self.get_disp(iconv2)
            udisp2  = self.upsample_nn(self.disp2[branch], 2)

            upconv1 = upconv(iconv2,  16, 3, 2) #H
            concat1 = tf.concat([upconv1, udisp2], 3)
            iconv1  = conv(concat1,   16, 3, 1)
            self.disp1[branch] = self.get_disp(iconv1)

        if branch == 'right':
            self.disp1[branch] = tf.reverse(self.disp1[branch], [2])
            self.disp2[branch] = tf.reverse(self.disp2[branch], [2])
            self.disp3[branch] = tf.reverse(self.disp3[branch], [2])
            self.disp4[branch] = tf.reverse(self.disp4[branch], [2])

    def build_resnet50(self, branch):
        #set convenience functions
        conv   = self.conv
        if self.params.use_deconv:
            upconv = self.deconv
            ...
            with tf.variable_scope('model', reuse=self.reuse_variables):
                ...
                if self.params.encoder == 'vgg':
                    with tf.variable_scope('vgg') as scope:
                        self.build_vgg(branch='left')
                        scope.reuse_variables()
                        self.build_vgg(branch='right')

                elif self.params.encoder == 'resnet50':
                    with tf.variable_scope('resnet50') as scope:
                        self.build_resnet50(branch='left')
                        scope.reuse_variables()
                        self.build_resnet50(branch='right')
                else:
                    raise ValueError("Unknown encoder: {}!".format(self.params.encoder))
    ...
    def build_summaries(self):
        ...
        for i in range(4):
            ...
            tf.summary.image('right_to_left_disp' + str(i), self.right_to_left_disp[i], max_outputs=4)
            tf.summary.image('left_to_right_disp' + str(i), self.left_to_right_disp[i], max_outputs=4)
